...
funcion_conteo_protestas_macrocomuna <- function(ruta_base_datos, p_comuna, dir_salida){
  
  macro_protestas_comuna <- st_read(dsn = ruta_base_datos, layer = p_comuna)
  ...
  write.csv(resultados_por_grupo_test, paste(dir_salida,"/", p_comuna,  "_protestas_tiempo.csv", sep = ""))
